...
graph = dcc.Graph(id="chart", figure={})
...
button = dbc.Button(
    "Plot Graph", id="draw", color="primary", className="mt-3 w-100", n_clicks=0
)
...
app.layout = dbc.Container(
    [
        title,
        dbc.Row(
            [create_card("Input Data", data_table, 5), create_card("Chart", graph, 7)],
            className="g-3",
        ),
        dbc.Row(
...
def plot_table(n_clicks, table_data):
    ...
    if not n_clicks:
        # Aucun clic, retourne un graphique vide
        return {}
