import dash  # nécessaire pour dash.no_update
...
df = px.data.tips()
# Détection des colonnes numériques utilisables
numeric_columns = [
...
data_table = dash_table.DataTable(
    ...
    columns=[
        {"name": i, "id": i, "selectable": i in numeric_columns} for i in df.columns
    ],
    editable=True,
...
@callback(
    ...
    Output("input-data", "active_cell"),
    Output("input-data", "selected_cells"),
    Input("input-data", "active_cell"),
    Input("input-data", "selected_columns"),
    Input("input-data", "data"),
)
def update_chart_message_and_active_cell(cell, col, table_data):
    """
    Met à jour le graphique, le message et désélectionne la cellule active si une colonne est sélectionnée.
    """
    triggered_prop = (
    ...
    if triggered_prop == "active_cell" and cell and dff.shape[0] > 0:
        ...
        message = html.Div(
            [
                html.P(
                    f"{ICON_POINTER} Active cell: Row {row}, Column '{col_id}'",
                    className="mb-1",
                ),
                html.P(
                    f"{ICON_BULB} Value: {cell_value}",
    ...
    # La sélection ne peut être qu'une colonne numérique
    y_col = selected_cols[0] if selected_cols else DEFAULT_Y_COLUMN
    fig = make_figure(dff, y_col)

    # Désélectionner la cellule active uniquement si la sélection de colonne a déclenché le callback
    if triggered_prop == "selected_columns" and cell is not None:
        return fig, message, None, []
    return fig, message, dash.no_update, dash.no_update
