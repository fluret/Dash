...
with open(pathlib.Path(__file__).parent / "gapminder_description.md", encoding="utf-8") as f:
        ...
        dbc.CardBody(
            [
                ...
                dbc.Button([
                    ...
                    "Informations"
                ], id="info-btn", color="primary", className="mt-3 w-100"),
            ]
        )
    ],
...
def update_graph(metric):
    ...
    label = next((opt['label'] for opt in METRIC_OPTIONS if opt['value'] == metric), metric)
    detail = metric_details.get(metric, "")
    toast = dbc.Toast(
        [
            html.Strong(f"Métrique affichée : {label}"),
            html.Br(),
            html.Span(detail)
        ],
        id="metric-toast",
        header=html.Span([html.Span(className="bi bi-info-circle-fill me-2"),
